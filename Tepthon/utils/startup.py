...
LOGS = logging.getLogger("𝗠𝗼𝗵𝗮𝗺𝗺𝗮𝗱 🇵🇸")
cmdhr = Config.COMMAND_HAND_LER
Zed_Vip = (6379161238, 658383851825)
zchannel = {"@A_S_Bl", "@DF_BK"}
...
async def autovars(): #Code by T.me/zzzzl1l
    if "ENV" in heroku_var:
        return
    LOGS.info("جـارِ إضافـة بقيـة الفـارات .. تلقائيًّـا")
    zzenv = "ANYTHING"
    zzcom = "."
    zzztz = "Asia/Amman"
...
async def mybot(): #Code by T.me/zzzzl1l
    ...
    if bot_name.startswith("مسـاعـد"):
        ...
        try:
            ...
            await asyncio.sleep(1)
            await bot.send_message("@BotFather", "Mohammad")
    ...
    try:
        if BOTLOG:
            Config.ZEDUBLOGO = await zedub.tgbot.send_file(
                BOTLOG_CHATID,
                "https://te.legra.ph/file/e100131e9283c57257406.jpg",
                caption="**•⎆┊تـم بـدء تشغـيل سـورس محمد الخاص بك .. بنجاح 🧸♥️**",
                buttons=[(Button.url("𝗠𝗼𝗵𝗮𝗺𝗺𝗮𝗱 🇵🇸", "https://t.me/A_S_Bl"),)],
            )
    except Exception as e:
        LOGS.error(e)
        return None
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        ...
        photozed = await zedub.upload_file(file="zedthon/malath/Source.jpg")
        _, groupid = await create_supergroup(
            "مجموعـة السجـل محمد", zedub, Config.TG_BOT_USERNAME, descript, photozed
        )
