...
ZelzalVP_cmd = (
    "𓆩 [𝗠𝗼𝗵𝗮𝗺𝗺𝗮𝗱 𝗖𝗼𝗻𝗳𝗶𝗴 𝗩𝗮𝗿𝘀 - أوامـر الفـارات](t.me/Tepthon) 𓆪\n\n"
    "**⎉╎قائمـة أوامر تغييـر فـارات الصـور بأمـر واحـد فقـط - لـ أول مـرة ع سـورس محمد يوزر بـوت 🦾 :** \n\n"
    ...
    "\n𓆩 [𝗠𝗼𝗵𝗮𝗺𝗺𝗮𝗱 𝗩𝗮𝗿𝘀 - قنـاة الفـارات](t.me/A_S_Bl) 𓆪"
)
...
async def variable(event):
    ...
    if input_str == "كليشة الفحص" or input_str == "كليشه الفحص":
        ...
        if variable in heroku_var:
            await zed.edit("**⎉╎تم تغييـر {} بنجـاح ☑️**\n**⎉╎المتغيـر : ↶**\n `{}` \n**⎉╎يتم الآن إعادة تشغيـل بـوت محمد يستغـرق الأمر 2-1 دقيقـة ▬▭ ...**".format(input_str, vinfo))
        else:
            await zed.edit("**⎉╎تم إضافة {} بنجـاح ☑️** \n**⎉╎المضاف اليه :**\n `{}` \n**⎉╎يتم الآن إعادة تشغيـل بـوت محمد يستغـرق الأمر 2-1 دقيقـة ▬▭ ...**".format(input_str, vinfo))
...
async def variable(event):
    ...
    if input_str == "كليشة الفحص" or input_str == "كليشه الفحص":
        ...
        await zed.edit("**⎉╎تم حـذف {} بنجـاح ☑️**\n**⎉╎المتغيـر المحـذوف : ↶**\n `{}` \n**⎉╎يتم الآن إعادة تشغيـل بـوت محمد يستغـرق الأمر 2-1 دقيقـة ▬▭ ...**".format(input_str, variable))
        ...
        if variable not in heroku_var:
        	return await zed.edit("**⎉╎عـذࢪًا عـزيـزي .. انت لـم تقـم بإضـافـة فـار {} اصـلاً...**".format(input_str))
        await zed.edit("**⎉╎تم حـذف {} بنجـاح ☑️**\n**⎉╎المتغيـر المحـذوف : ↶**\n `{}` \n**⎉╎يتم الآن إعادة تشغيـل بـوت محمد يستغـرق الأمر 2-1 دقيقـة ▬▭ ...**".format(input_str, heroku_var[variable]))
...
async def variable(event):
    ...
    viran = "Asia/Tehran"
    heroku_var = app.config()
    zed = await edit_or_reply(event, "**⎉╎جـاري إعـداد المنطقـة الزمنيـة لـ محمد 🌐...**")
    # All Rights Reserved for "Zed-Thon" "زلـزال الهيبـة"
    if input_str == "العراق" or input_str == "اليمن" or input_str == "سوريا" or input_str == "السعودية" or input_str == "لبنان" or input_str == "الاردن" or input_str == "فلسطين":
        ...
        await asyncio.sleep(1.5)
        if variable in heroku_var:
            await zed.edit("**⎉╎تم تغييـر المنطقـة الزمنيـة .. بنجـاح ☑️**\n**⎉╎المتغير : ↶**\n دولـة `{}` \n**⎉╎يتم الآن إعادة تشغيـل بـوت محمد يستغـرق الأمر 2-1 دقيقـة ▬▭ ...**".format(input_str))
        else:
