...
async def supc(event):  # Zed-Thon - ZelZal
    ...
    zed_dev = (6379161238, 92542972505, 51756749470, 52801339206)
            ...
            if "not found" in reqt:
                try:
                    ...
                    if chn.startswith("https://"):
                        await event.reply(f"**⎆╎يجب عليك الاشـتࢪاڪ بالقناة أولًا\n⎆╎قناة الاشتراك : {chn}**", buttons=[[Button.url("اضغط لـ الاشـتࢪاڪ 🤍", chn)]]
                        )
                        return await event.delete()
                    else:
                        await event.reply(f"**⎆╎اشترك في قـنـاة الاشـتراك للتحدث معي رجـاءً لا يمكنك التـحـدث إلا بـعد الاشـتراك ♡\n⎆╎قناة الاشتراك : @{chn} **", buttons=[[Button.url("اضغط لـ الاشـتࢪاڪ 🤍", f"https://t.me/{chn}")]]
                        )
                        return await event.delete()
