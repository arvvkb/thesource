...
async def amireallyalive(event):
    ...
    start = datetime.now()
    zedevent = await edit_or_reply(event, "**✦ جـاري فحـص تنصيبـك لـ 𝗠𝗼𝗵𝗮𝗺𝗺𝗮𝗱 ⌬ . .**")
    end = datetime.now()
    ...
    ALIVE_TEXT = gvarstatus("ALIVE_TEXT") or "** بـوت محمد 𝗠𝗼𝗵𝗮𝗺𝗺𝗮𝗱 يعمـل بنجـاح ☑️ .. **"
...
**{Z_EMOJI} قنـاة الـسـورس :** [اضغـط هنـا](https://t.me/A_S_Bl)"""
...
async def amireallyialive(event):
    ...
    reply_to_id = await reply_id(event)
    Z_EMOJI = gvarstatus("ALIVE_EMOJI") or "⎆╿"
    zed_caption = "** بـوت محمد 𝗠𝗼𝗵𝗮𝗺𝗺𝗮𝗱 يعمـل بنجـاح ☑️ .. **\n"
